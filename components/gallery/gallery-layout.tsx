...
export function GalleryLayout() {
	...
	const groupedByDay = galleryImages.reduce(
		(acc, image) => {
			if (!acc[image.day]) {
				acc[image.day] = [];
			}
	...
	return (
		<div className="relative w-full min-h-screen bg-background text-foreground h-full scrollbar-hide overflow-y-scroll" ref={containerRef}>
			...
			<div className="container mx-auto px-4 py-12 md:py-24">
				...
				>
					<h1 className="text-4xl md:text-6xl font-bold tracking-tight mb-6">A Day in Kyoto</h1>
									...
									<div key={item.id} className="flex flex-col gap-4 mb-6">
										<div className="flex items-start justify-between gap-4">
											...
											<div className="flex items-center gap-2">
												<Button variant="outline" size="icon" onClick={() => updateQuantity(item.id, -1)} disabled={item.quantity <= 1}>
			...
			<AnimatePresence mode="wait">
				{selectedImage && <ExpandedImage image={selectedImage} clickPosition={clickPosition} onClose={handleClose} />}
			</AnimatePresence>
		</div>
	);
}
